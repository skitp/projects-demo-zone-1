# Databricks / Microsoft Fabric Notebook
# Title: Delta Table Health Metrics & Cost-Aware Auto-OPTIMIZE
# Description: Fabric-native equivalent of sys.sp_get_table_health_metrics.
...
#              benefit clearly justifies the compute cost.
#
...
# MAGIC %md
# MAGIC # Delta Table Health Assessment & Cost-Aware OPTIMIZE
# MAGIC 
# MAGIC This notebook implements a production-grade, config-driven health scoring
# MAGIC engine for Delta tables. It is the Fabric-native counterpart to
# MAGIC `sys.sp_get_table_health_metrics`.
# MAGIC 
# MAGIC **Key behaviours**
# MAGIC - Collects real metrics via `DESCRIBE DETAIL` + `DESCRIBE HISTORY`
# MAGIC - Applies weighted scoring rules from `table_health_config.json`
# MAGIC - Produces clear recommendations: `HEALTHY` / `MONITOR` / `OPTIMIZE_SOON` / `OPTIMIZE_NOW`
# MAGIC - Auto-OPTIMIZE is **gated** by table size, file count, average file size **and** score threshold
# MAGIC - Results are upserted daily into `housekeeping.table_health_metrics`
# MAGIC - Safe by default (`dry_run = true`, `allow_auto_optimize = false`)

# COMMAND ----------

# MAGIC %md
# MAGIC ## 1. Parameters & Configuration
...
# ------------------------------------------------------------------------------
# Notebook parameters (Fabric / Databricks widgets)
# ------------------------------------------------------------------------------
# Works with both Databricks dbutils and Fabric notebookutils.
# Defaults are intentionally safe for production (dry_run=true).

def _get_param(name: str, default: str) -> str:
    """Retrieve a notebook parameter with graceful fallback."""
    try:
        # Databricks style
        ...
        pass
    try:
        # Fabric notebookutils style (newer runtimes)
        from notebookutils import mssparkutils
        return mssparkutils.notebook.getContext().get(name, default) or default
    except Exception:
        pass
...
# Register widgets when dbutils is available (Databricks / some Fabric runtimes)
try:
    dbutils.widgets.text("config_path", "Files/table_health_config.json", "Path to config JSON")
    dbutils.widgets.text("target_schemas", "iis_transaction,claims_transaction,stg_claims_transaction", "Comma-separated schemas")
    dbutils.widgets.text("table_filter", "", "Optional table name filter")
    dbutils.widgets.text("dry_run", "true", "true = assess only")
    dbutils.widgets.text("force_optimize", "false", "true = ignore allow_auto_optimize")
except Exception:
    pass  # Widgets already exist or pure Fabric environment

CONFIG_PATH          = _get_param("config_path", "Files/table_health_config.json")
TARGET_SCHEMAS_RAW   = _get_param("target_schemas", "iis_transaction,claims_transaction,stg_claims_transaction")
TABLE_FILTER_RAW     = _get_param("table_filter", "")
DRY_RUN              = _get_param("dry_run", "true").lower() == "true"
FORCE_OPTIMIZE       = _get_param("force_optimize", "false").lower() == "true"

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2. Logging & Spark Session
...
if spark is None:
    raise RuntimeError("No active SparkSession. Run this notebook inside a Fabric / Databricks Spark environment.")
...
def load_config(path: str) -> Dict[str, Any]:
    """
    Load table_health_config.json.
    Supports:
      - Absolute / relative filesystem paths
      - Fabric lakehouse Files/ paths
      - abfss:// OneLake URIs (via fsspec if available)
    """
    candidates = [
        path,
    ...
    # Last resort – try fsspec for abfss
    try:
        import fsspec
...
# MAGIC %md
# MAGIC ## 4. Core Scoring Functions (exact logic from reference)
...
def calculate_health_score(
    ...
    score += dml_score
    if dml_score > 0:
        reasons.append(f"{recent_dml_count} MERGE/UPDATE/DELETE operations (last {DML_LOOKBACK_DAYS}d)")
...
# MAGIC %md
# MAGIC ## 5. Metric Collection from Delta Tables
...
def get_history_metrics(full_table_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if optimize_rows:
            ...
            days_since = (datetime.now(timezone.utc) - last_opt_ts.replace(tzinfo=timezone.utc)).days
        else:
            days_since = 9999  # never optimized

        # Recent DML (MERGE, UPDATE, DELETE, WRITE, CREATE OR REPLACE)
        cutoff = datetime.now(timezone.utc) - timedelta(days=DML_LOOKBACK_DAYS)
        dml_ops = {"MERGE", "UPDATE", "DELETE", "WRITE", "CREATE OR REPLACE TABLE AS SELECT", "CREATE TABLE AS SELECT"}
        recent_dml = (
            hist_df
        ...
        logger.warning("History analysis failed for %s: %s. Using conservative defaults.", full_table_name, str(e))
        return {
...
# MAGIC %md
# MAGIC ## 6. Metrics Table Management
...
# MAGIC %md
# MAGIC ## 7. OPTIMIZE Execution (cost-aware)
...
def discover_tables(schemas: List[str], table_filter: str = "") -> List[str]:
    ...
    """
    tables = []
    filter_set = {t.strip().lower() for t in table_filter.split(",") if t.strip()} if table_filter else set()
...
def assess_single_table(
    ...
    try:
        ...
        # Decide whether to actually run OPTIMIZE
        ...
        logger.info("  score=%d | recommendation=%s | optimize_required=%s", score, recommendation, optimize_req)
        for r in reasons:
            logger.info("    • %s", r)
        ...
        if will_optimize:
            ...
            logger.info("  [DRY-RUN] OPTIMIZE would be executed (score=%d >= threshold)", score)
        elif optimize_req and not allow_auto:
            logger.info("  OPTIMIZE warranted but allow_auto_optimize=false (and force_optimize=false)")
...
def main() -> None:
    ...
    logger.info("=" * 80)
    logger.info("Delta Table Health Assessment started | run_id=%s | dry_run=%s | force_optimize=%s",
    ...
    schemas = [s.strip() for s in TARGET_SCHEMAS_RAW.split(",") if s.strip()]
    if not schemas:
        # Fallback: all schemas the current user can see
    ...
    table_list = discover_tables(schemas, TABLE_FILTER_RAW)
    if not table_list:
    ...
    for tbl in table_list:
        row = assess_single_table(tbl, dry_run=DRY_RUN, force_optimize=FORCE_OPTIMIZE, run_id=run_id)
        if row is not None:
            results.append(row)
            # Upsert immediately so partial progress is saved
            try:
    ...
    if results:
        ...
        if now:
            ...
            for r in sorted(now, key=lambda x: -x.health_score):
                logger.info("  %-50s score=%3d  %s", r.table_name, r.health_score, ", ".join(r.reason_codes[:2]))
    logger.info("=" * 80)
...
if __name__ == "__main__" or True:          # Always run when the notebook is executed
    try:
        main()
    except Exception as e:
...
# MAGIC %md
# MAGIC ## 11. Optional – Quick Health Dashboard Query
# MAGIC 
# MAGIC ```sql
# MAGIC SELECT
# MAGIC   table_name,
...
# MAGIC   size_gb,
# MAGIC   num_files,
# MAGIC   avg_file_size_mb,
# MAGIC   days_since_optimize,
# MAGIC   recent_dml_count,
# MAGIC   optimize_required,
# MAGIC   optimize_executed,
# MAGIC   reason_codes,
# MAGIC   assessment_date
# MAGIC FROM housekeeping.table_health_metrics
# MAGIC WHERE assessment_date >= current_date() - 7
# MAGIC ORDER BY health_score DESC, assessment_date DESC;
# MAGIC ```
