...
def create_dataset_files(mode: Literal["cdc", "incremental"] = "incremental") -> None:
    ...
    Metadata sources:
      ...
      - table_ext_properties_crqry (ms_description + domain_owner)
    ...
    # ------------------------------------------------------------------
    # 4. Extended properties + ownership metadata (shared)
    # ------------------------------------------------------------------
    dl_table_path_sdr = f"{lhs_path_root}/Tables/catalog/source_dataset_request"
    ...
    query = f"""
        ...
        LEFT JOIN delta.`{dl_table_path_exp}` AS exp
            ...
            AND exp.extended_property_name = 'ms_description'
        WHERE sdr.status = 'initiated'
    """
    logger.info(f"Loading extended properties + domain_owner from {dl_table_path_exp}")
    exp_dataset_df: DataFrame = spark.sql(query)

    # ------------------------------------------------------------------
    # 5. Process each requested dataset
    # ------------------------------------------------------------------
    generated_count = 0
    skipped_count   = 0
    ...
    for row in datasets_to_process:
        ...
        if ds_curated.isEmpty():
            logger.warning(
                f"Skipping {database_name}.{dataset_schema}.{dataset_name}: "
                "no columns found in source_dataset_column_list"
            )
            skipped_count += 1
            continue
        ...
        # ---- target load type ----
        # Explicit rule: empty primary_key_list always → "overwrite"
        if not primary_key_list:
            target_load_type = "overwrite"
        elif ingest_type != "full":
            target_load_type = "merge"
        else:
            target_load_type = "overwrite"

        # ---- extended properties + domain owner ----
        exp_row = (
            exp_dataset_df
            .filter(
                (F.col("database_name") == database_name) &
                (F.col("schema_name")   == dataset_schema) &
                (F.col("table_name")    == dataset_name)
            )
            .first()
        )
        ...
        if exp_row:
            description  = exp_row["extended_property_value"]
            domain_owner = exp_row["domain_owner"]
        ...
        config: Dict[str, Any] = {
            ...
            "curatedProperties": {
                ...
                "deltaTableProperties": {
                    ...
                    "tblproperties": {
                        "governance.data_domain": domain_owner,
