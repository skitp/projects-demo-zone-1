"""
Ingest.py - Production Ingest class for Microsoft Fabric lakehouse pipelines.

Enhanced with apply_governance_metadata() method for standardized
Delta table governance (TBLPROPERTIES + table/column comments).
"""
...
class Ingest:
    ...
    def __init__(self, ingest_config: str) -> None:
        ...
        # Governance / Delta table properties (new section)
        self.delta_table_properties = (
            config.get("deltaTableProperties")
            or config.get("curatedProperties", {}).get("deltaTableProperties")
            or {}
        )
    ...
    def start_ingest(self, elt_id: str = "0", run_id: str = "0") -> None:
        ...
        try:
            ...
            if expectation_success:
                ...
                # ============================================================
                # NEW: Apply governance metadata after successful load
                # ============================================================
                try:
                    self.apply_governance_metadata(self.delta_table_properties)
                except Exception as gov_e:
                    self.logger.warning(f"Governance metadata application failed (non-fatal): {gov_e}")
    ...
    # ============================================================
    # NEW METHOD: apply_governance_metadata()
    # ============================================================
    def apply_governance_metadata(self, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
        Apply governance metadata to the target Delta table.

        This includes:
        - Table-level comment (description)
        - Column-level comments (if provided)
        - Standardized governance TBLPROPERTIES for discoverability, ownership,
          PII classification, and auditability.

        Can be called manually or is automatically invoked at the end of a
        successful start_ingest() run.

        Args:
            metadata: Optional dict with governance information.
                      If not provided, values from the "deltaTableProperties" section
                      in ingest_config are used (merged with sensible defaults).
        """
        full_table_name = f"{self.target_schema}.{self.target_table}"

        # Ensure target lakehouse is initialized
        if not self._target_lakehouse:
            self.target_lakehouse(self.target_lakehouse_name, self._target_workspace_id)

        # Verify table exists
        if not self._target_lakehouse.check_if_table_exists(self.target_table, self.target_schema):
            self.logger.warning(f"Table {full_table_name} does not exist yet. Skipping governance metadata.")
            return

        # Build sensible defaults, then overlay any values from deltaTableProperties in config
        defaults = {
            "description": f"Curated {self.load_type} table from {self.source_system} - {self.target_table}",
            "owner": "data-engineering",
            "source_system": self.source_system,
            "pii_classification": "UNKNOWN",
            "refresh_cadence": self.load_type.lower(),
        }

        if metadata:
            # User-provided values (from deltaTableProperties) take precedence
            defaults.update(metadata)
        metadata = defaults

        self.logger.info(f"Applying governance metadata to {full_table_name}")

        try:
            # 1. Table comment
            if metadata.get("description"):
                safe_desc = str(metadata["description"]).replace("'", "''")
                spark.sql(f"COMMENT ON TABLE {full_table_name} IS '{safe_desc}'")
                self.logger.info(f"  ✓ Table comment applied")

            # 2. Column comments (optional)
            column_comments: Dict[str, str] = metadata.get("column_comments", {})
            applied_col_comments = 0
            source_columns_lower = [c.lower() for c in getattr(self, '_source_data', type('obj', (object,), {'columns': []})()).columns]
            for col_name, comment in column_comments.items():
                if comment and col_name.lower() in source_columns_lower:
                    safe_comment = str(comment).replace("'", "''")
                    try:
                        spark.sql(f"ALTER TABLE {full_table_name} ALTER COLUMN {col_name} COMMENT '{safe_comment}'")
                        applied_col_comments += 1
                    except Exception as col_e:
                        self.logger.debug(f"Could not set comment on column '{col_name}': {col_e}")
            if column_comments:
                self.logger.info(f"  ✓ Column comments applied ({applied_col_comments}/{len(column_comments)})")

            # 3. Governance TBLPROPERTIES (core value)
            tbl_props: Dict[str, str] = {
                "governance.owner": metadata.get("owner", "unknown"),
                "governance.source_system": self.source_system,
                "governance.pii_classification": metadata.get("pii_classification", "UNKNOWN"),
                "governance.refresh_cadence": metadata.get("refresh_cadence", self.load_type.lower()),
                "governance.last_updated_utc": datetime.now(timezone.utc).isoformat(),
                "governance.ingest_class_version": "2.0",
                "governance.load_type": self.load_type,
                "governance.dataset_type": self.dataset_type_name,
            }

            # Allow caller to override / add custom properties
            if "tblproperties" in metadata and isinstance(metadata["tblproperties"], dict):
                tbl_props.update(metadata["tblproperties"])

            # Build and execute ALTER TABLE SET TBLPROPERTIES
            props_clause = ", ".join(
                [f"'{k}' = '{str(v).replace(chr(39), chr(39)*2)}'" for k, v in tbl_props.items()]
            )
            spark.sql(f"ALTER TABLE {full_table_name} SET TBLPROPERTIES ({props_clause})")

            self.logger.info(f"  ✓ Governance TBLPROPERTIES applied ({len(tbl_props)} properties)")

        except Exception as e:
            self.logger.error(f"Failed to apply governance metadata to {full_table_name}: {e}")
            if self.raise_errors:
                raise
            else:
                self.logger.warning("Governance metadata application failed but raiseErrors=False — continuing.")
...
if __name__ == "__main__":
    print("Ingest class (with apply_governance_metadata) loaded successfully.")
